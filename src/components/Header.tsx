...
const Header = () => {
  ...
  return <header className="sticky top-0 z-50 bg-black backdrop-blur-xl" style={{ backgroundImage: "radial-gradient(circle at 50% 0%, rgba(37, 99, 235, 0.14), transparent 42%), linear-gradient(90deg, #000000, #09090b 50%, #000000)" }}>
    ...
    {open && <nav id="mobile-navigation" aria-label="Mobile navigation" className="page-shell border-t border-zinc-800 py-3 md:hidden">{navLinks.map((link) => <Link key={link.title} href={link.url} onClick={() => setOpen(false)} className="block rounded-xl px-4 py-3 text-base font-medium text-zinc-300 hover:bg-zinc-900 hover:text-white">{link.title}</Link>)}</nav>}
  </header>;
};
